module.exports = {
    ...
    url_resource_reg: /^((resource|mockSys)\/|static\/|favicon\.ico|robot\.txt)/, //判断是否是静态资源的正则
    ...
    default_controller: 'H', //默认模块
    default_action: 'infUpLoad',  //默认Action
    call_controller: 'Home:H:_404', //controller不存在时执行方法，此配置表示调用Home分组下IndexController的_404Action方法
    ...
    auto_send_content_type: false, //是否自动发送Content-Type,默认值为`tpl_content_type`配置值
    ...
    log_memory: true, //记录内存使用和负载
    log_memory_path: LOG_PATH + '/memory', //日志文件存放路径
    log_memory_interval: 1000, //一分钟记录一次
